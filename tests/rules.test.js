...
test('rulebook explains the full-round sixteen-turn contest and special targeting', () => {
  assert.match(html, /opponent.*repl(?:y|ies).*before.*judg/is);
  assert.match(html, /cannot end before turn 12/i);
  assert.match(html, /20-point lead.*domination/i);
  assert.match(html, /turn 16.*leader wins/is);
